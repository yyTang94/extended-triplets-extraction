"Provides basic training and validation with `MyLearner`"
...
class LossFnRetureZero(object):

    def __init__(self):
        pass

    def __call__(self, score, yb_matrix):

        return (torch.tensor(0.0)).to(score.dtype).to(score.device)
...
def validate(model:nn.Module, dl:DataLoader, loss_func:OptLossFunc=None, cb_handler:Optional[CallbackHandler]=None,
    ...
    with torch.no_grad():
        ...
        for xb,yb in progress_bar(dl, parent=pbar, leave=(pbar is not None)):
            ...
            val_losses.append(loss_batch(model, xb, yb, loss_func=LossFnRetureZero(), cb_handler=cb_handler))
            if not is_listy(yb): yb = [yb]
...
class MyLearner():
    ...
    def export(self, fname:str='export.pkl'):
        "Export the state of the `MyLearner` in `self.path/fname`."
    ...
    def purge(self, clear_opt:bool=True):#TODO: opt
        "Purge the `MyLearner` of all cached attributes to release some GPU memory."
        path = self.path
        args = ['opt_func', 'loss_func', 'metrics', 'true_wd', 'bn_wd', 'wd', 'train_bn', 'model_dir', 'callback_fns']
        state = {a:getattr(self,a) for a in args}
        state['cb_state'] = {cb.__class__:cb.get_state() for cb in self.callbacks}
...
class LearnerCallback(Callback):
    "Base class for creating callbacks for a `MyLearner`."
    learn: MyLearner
    ...
    @property
    def learn(self) -> MyLearner: return self._learn()
    @learn.setter
    def learn(self, learn: MyLearner) -> None: self._learn = weakref.ref(learn)
...
class Recorder(LearnerCallback):
    ...
    _order=-10
    def __init__(self, learn:MyLearner):
        super().__init__(learn)
        self.opt = self.learn.opt
        self.train_dl = self.learn.data.train_dl
        self.no_val,self.silent = False,False
        self.path_of_recorder = str(self.learn.path) + '/'
    ...
    def on_backward_begin(self, smooth_loss:Tensor, **kwargs:Any)->None:
        ...
        self.losses.append(smooth_loss.detach().cpu())
    ...
    def on_epoch_end(self, epoch:int, num_batch:int, smooth_loss:Tensor,
        ...
        if (epoch % 300) == 0:
            ...
            valid_losses = [x.tolist()
                            for x in self.val_losses]
            prf_values = [x[1:] for x in self.metrics]
            inout.write(train_losses, valid_losses, prf_values, self.path_of_recorder)
        #
...
def load_callback(class_func, state, learn:MyLearner):
...
def load_learner(path:PathOrStr, fname:PathOrStr='export.pkl', test:ItemList=None):
    "Load a `MyLearner` object saved with `export_state` in `path/fn` with empty data, optionally add `test` and load on `cpu`."
