...
class LossFnRetrunZero(object):

    def __init__(self):
        pass

    def __call__(self, score, yb):
...
def validate(model:nn.Module, dl:DataLoader, loss_func:OptLossFunc=None, cb_handler:Optional[CallbackHandler]=None,
    ...
    with torch.no_grad():
        ...
        for xb,yb in progress_bar(dl, parent=pbar, leave=(pbar is not None)):
            ...
            val_losses.append(loss_batch(model, xb, yb, loss_func=LossFnRetrunZero(), cb_handler=cb_handler))
            if not is_listy(yb): yb = [yb]
    ...
    def on_epoch_end(self, epoch:int, num_batch:int, smooth_loss:Tensor,
        ...
        if (epoch % 300) == 0:
            ...
            inout.write(train_losses, valid_losses,
                        acc_values, prf_values, self.path_of_recorder)
        #
